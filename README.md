...
Individual post URLs under `https://www.agroverse.shop/post/<slug>` are listed in `sitemap.xml` inside the `<!-- BEGIN_AUTO_BLOG_POSTS -->` … `<!-- END_AUTO_BLOG_POSTS -->` block, which is **regenerated by script** from every `post/<slug>/index.html`.
